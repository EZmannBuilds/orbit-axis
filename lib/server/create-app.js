...
import { handleTarotRoute } from "../tarot/api.js";
import { handleOrbitXRoute, orbitXEnabled } from "../orbit-x/api.js";
import { handleBillingRoute, handleStripeWebhook } from "../billing/api.js";
...
import { createMemoryRateLimiter, RATE_LIMITS, rateLimitKey } from "../api/rate-limit.js";
import { handleAnalyticsRoute } from "../analytics/api.js";
import { orbitXStore } from "../orbit-x/store.js";
...
// ── Rate limiting for the open calculation routes (Dev Update 6.0) ──────────
//
// /api/v1/* has had a limiter since it shipped. These older sibling routes did
// not, and two of them are the expensive ones: /api/chart and /api/chart/now
// spawn the Swiss Ephemeris subprocess, and every /api/tarot/* path is reachable
// without an account. Unlimited, they are a free compute endpoint with Orbit
// paying the bill.
//
// HONEST SCOPE: this reuses lib/api/rate-limit.js, which is IN-MEMORY and
// PER-INSTANCE. Vercel runs many instances, so the effective global ceiling is
// roughly (limit x instances). It stops runaway clients and casual abuse — the
// realistic threat for a free beta — and it is NOT protection against a
// determined distributed attacker. Swapping in a shared store is a one-file
// change behind the same interface; it is deliberately not done here because it
// would add a paid infrastructure dependency.
const legacyComputeLimiter = createMemoryRateLimiter(RATE_LIMITS.calculation);
let legacyComputeChecks = 0;

/** Routes that cost real work and answer without an account. */
function isRateLimitedComputeRoute(route) {
  return route === "/api/chart"
    || route === "/api/chart/now"
    || route === "/api/events"
    || route === "/api/stella/daily"
    || route.startsWith("/api/tarot/")
    // The analytics beacon is open by necessity; a limiter is the only thing
    // standing between it and a trivially scriptable row generator.
    || route === "/api/analytics/event";
}

/**
 * One 429 shape for every limited legacy route, matching the {ok,error,code}
 * envelope those routes already use, plus the Retry-After header a well-behaved
 * client honours.
 */
async function refuseIfRateLimited(req, res, route) {
  if (!isRateLimitedComputeRoute(route)) return false;
  // Keyed by hashed peer address: these routes answer signed-out visitors, so
  // there is usually no account to key on, and a raw IP is personal data.
  const key = await rateLimitKey(req);
  const verdict = legacyComputeLimiter.check(key);
  // Bounded memory without a timer: prune on a cheap cadence instead.
  if ((legacyComputeChecks += 1) % 200 === 0) legacyComputeLimiter.prune();
  if (verdict.allowed) return false;
  json(res, 429, {
    ok: false,
    error: "That is a lot of requests in a short time. Please wait a moment and try again.",
    code: "rate_limited",
...
function createRequestHandler(env) {
  ...
  return async function orbitRequestHandler(req, res) {
    ...
    if (await refuseIfRateLimited(req, res, route)) return;
    ...
    try {
      ...
      // Tarot. Gated twice over: the feature flag decides whether the routes
      // exist at all, and the deck's own readiness decides whether a card can
      // be served — so even an enabled instance with an unauthored deck
      // refuses rather than improvising. Reads are open (a daily card is the
      // same for every signed-out visitor and is stored nowhere); saving and
      // history require a session and are owner-scoped through RLS.
      if (route.startsWith("/api/tarot/")) {
        // No feature-flag check. Tarot graduated out of the registry when it
        // finished, and featureEnabled() answers false for a name it does not
        // know — so leaving the check here 404'd every Tarot route the moment
        // the flag was removed. The gate that remains is the deck's own
        // readiness, enforced in the service on every read path.
        const needsAccount = route === "/api/tarot/readings";
        let auth = null;
        if (needsAccount) {
          auth = await requireAuth(req, res, env);
          if (!auth) return;
        } else {
          // Optional: a signed-in reader gets their own daily card, a visitor
          // gets the day's shared one. Neither path refuses.
          const attempted = await authenticateRequest(req, env);
          auth = attempted?.ok ? attempted : null;
        }
        const body = req.method === "POST" ? await readBody(req) : {};
        const handled = await handleTarotRoute(req.method, route, url.searchParams, body, {
      ...
      // The write path is open: the visitors worth counting are the ones who
      // have not signed up. Auth is read OPTIONALLY so an event from a signed-in
      // reader can be attributed to the account, and never refused if it is not.
      // The read path is the beta metrics panel and is admin-only.
      if (route.startsWith("/api/analytics/")) {
        const attempted = await authenticateRequest(req, env);
        const auth = attempted?.ok ? attempted : null;
        const context = authContext(auth);

        let isAdmin = false;
        if (route === "/api/analytics/metrics" && context?.ownerId) {
          // The same membership fact RLS enforces on the rows themselves, asked
          // once here so the handler can answer 404 without a partial read.
          try { isAdmin = await orbitXStore(context).isAdmin(); } catch { isAdmin = false; }
        }

        const body = req.method === "POST" ? await readBody(req) : {};
        const handled = await handleAnalyticsRoute(req.method, route, body, { auth: context, isAdmin });
        if (handled) return json(res, handled.status, handled.body, withSession(req, auth?.setCookie));
      }

      // Orbit X internal editorial desk (Dev Update 5.0). Three gates in
      // order: the server-controlled flag (an unset production has no Orbit X
      // at all), the session, then admin membership checked inside the
      // handler against the orbit_x_admins table — the same fact RLS enforces
      // on every row. Non-admins get 404: an internal tool does not advertise
      // itself to accounts that cannot use it.
      if (route.startsWith("/api/orbit-x/")) {
        // The flag reads PROCESS env, not the resolved environment info the
        // handler's `env` variable carries — that object is classification
        // (isProduction, vercelEnv…), not raw variables, and the first wiring
        // of this block read the flag off it and was permanently off.
        if (!orbitXEnabled(process.env)) return json(res, 404, { ok: false, error: "Not found." });
        const auth = await requireAuth(req, res, env);
        if (!auth) return;
        const body = ["POST", "PATCH"].includes(req.method) ? await readBody(req) : {};
        const handled = await handleOrbitXRoute(req.method, route, url.searchParams, body, authContext(auth), { env: process.env });
        if (handled) return json(res, handled.status, handled.body, withSession(req, auth.setCookie));
      }
      // The desk's page itself lives OUTSIDE public/ on purpose: public/ is
      // the app bundle that ships inside the iOS binary, and an internal tool
      // has no business travelling in every customer's pocket. Served only
      // when the flag is on; the data behind it still requires an admin.
      if (route === "/admin/orbit-x" && req.method === "GET") {
        if (!orbitXEnabled(process.env)) { res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" }); return res.end("Not found"); }
        const page = await import("node:fs/promises").then((fs) =>
          fs.readFile(new URL("../orbit-x/ui.html", import.meta.url), "utf8"));
        res.writeHead(200, { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store" });
        return res.end(page);
      }
      // The desk's renderer modules (Dev Update 5.1): the same files Node
      // tests import, served verbatim so preview and export are pixel-equal
      // to what the tests assert. An explicit allow-list — no path arithmetic
      // reaches the filesystem, and api.js/store.js are NOT servable.
      {
        const assetMatch = route.match(/^\/admin\/orbit-x\/(celestial|templates|language|formats|schemas|editorial)\.js$/);
        if (assetMatch && req.method === "GET") {
          if (!orbitXEnabled(process.env)) { res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" }); return res.end("Not found"); }
          const module = await import("node:fs/promises").then((fs) =>
            fs.readFile(new URL(`../orbit-x/${assetMatch[1]}.js`, import.meta.url), "utf8"));
          res.writeHead(200, { "Content-Type": "text/javascript; charset=utf-8", "Cache-Control": "no-store" });
          return res.end(module);
        }
      }
      // Web billing (Dev Update 3.10). The three account routes require the
      // session like any other owner data. The webhook is UNAUTHENTICATED by
      // design — Stripe is not a signed-in user — and trusts nothing except
      // the HMAC signature verified over the RAW bytes, which is why it reads
      // readBytes() and never readBody(): a re-serialised JSON body would have
      // different bytes from the ones Stripe signed.
      if (route === "/api/stripe/webhook" && req.method === "POST") {
        const raw = await readBytes(req, 1024 * 1024);
        if (raw.tooLarge) return json(res, 413, { ok: false, error: "Event too large." });
        if (raw.aborted || !raw.bytes) return json(res, 400, { ok: false, error: "Event body incomplete." });
        const handled = await handleStripeWebhook(raw.bytes, req.headers["stripe-signature"], { env, req });
        return json(res, handled.status, handled.body);
      }
      if (route.startsWith("/api/billing/")) {
        const auth = await requireAuth(req, res, env);
        if (!auth) return;
        const body = req.method === "POST" ? await readBody(req) : {};
        const handled = await handleBillingRoute(req.method, route, body, authContext(auth), { req, env });
        if (handled) return json(res, handled.status, handled.body, withSession(req, auth.setCookie));
      }
