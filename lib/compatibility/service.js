...
function resolveMode(subject, other) {
  ...
  // NEITHER CHART IS THE ACCOUNT HOLDER'S OWN — decided FIRST, and this is the
  // whole fix. The relationship check below used to run ahead of it, so
  // comparing two friends failed whenever the second chart had no relationship
  // type set. That is most saved charts: the field describes how someone
  // relates to the OWNER, and a general comparison is not reading that
  // relationship at all. Demanding it was asking for a value the answer never
  // consults.
  //
  // The comparison is still real — geometry between two charts needs nobody's
  // permission — but the LABEL cannot travel with it, because calling this
  // "partner compatibility" would assert a relationship between two other
  // people that nobody has claimed exists.
  if (!subjectIsOwner) return "general";

  if (!isCalculableRelationship(raw)) {
    // Reading outward from the owner's own chart DOES need to know what the
    // other person is to them. `other` and NULL mean nobody chose, and
    // `public_figure` is a real choice that is not one of the four modes —
    // mapping any of them to a mode would invent a relationship on the
    // person's behalf.
    throw new CompatibilityError(
      "relationship_required",
      "This chart needs a relationship type before it can be compared with yours.",
      {
        chart_id: other.id,
        chart_name: other.nickname || null,
        stored_value: raw,
        // What a person can actually SET on a chart. `general` is a mode the
        // server derives, never a relationship anyone can choose, so offering
        // it would send someone looking for an option that does not exist.
  ...
  // Both sides Self: two of the owner's own configurations, usually the same
  // birth recorded with a different time or place. Never dressed up as a
  // relationship, because nobody is in one with themselves.
