...
export function orbitXStore(auth, { fetchImpl = fetch } = {}) {
  ...
  return {
    ...
    async history({ status = null, limit = 50 } = {}) {
      ...
      return rows(`orbit_x_posts?select=id,event_key,event_type,selected_format,status,editorial_score,rejection_reason,created_at,updated_at,approved_at,exported_at,edited_copy,event_payload,reading_type,period_key,period_start_at,period_end_at,template_family,template_version${filter}&order=created_at.desc&limit=${Math.min(Number(limit) || 50, 200)}`);
    },
