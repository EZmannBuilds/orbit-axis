...
export function orbitXStore(auth, { fetchImpl = fetch } = {}) {
  ...
  return {
    ...
    async history({ status = null, limit = 50 } = {}) {
      ...
      // edited_copy and event_payload ride along since Dev Update 5.1 so the
      // desk's history can render first-slide thumbnails — a content library,
      // not just a table of metadata.
      return rows(`orbit_x_posts?select=id,event_key,event_type,selected_format,status,editorial_score,rejection_reason,created_at,updated_at,approved_at,exported_at,edited_copy,editor_notes,event_payload,reading_type,period_key,period_start_at,period_end_at,template_family,template_version${filter}&order=created_at.desc&limit=${Math.min(Number(limit) || 50, 200)}`);
    },
    ...
    async coverageFor(eventKey) {
      return rows(`orbit_x_posts?event_key=eq.${encodeURIComponent(eventKey)}&status=in.(draft,approved,exported)&select=id,status,selected_format,created_at,updated_at&order=updated_at.desc&limit=10`);
    },
