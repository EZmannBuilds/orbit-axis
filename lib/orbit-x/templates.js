...
export function renderSlide(ctx) {
  ...
  if (role === "hero") {
    ...
    const hookFit = hookText
      ? fitText(hookText, { tiers: [38, 34, 30], maxLines: SAFE_LIMITS.hookLines, width: S.w - 20, font: "sans" })
      : null;
    if (hookFit?.overflow) {
      warnings.push(`The hook line is too long for slide 1 — keep it to about ${SAFE_LIMITS.hookChars} characters.`);
    }
    const hookH = hookFit ? hookFit.lines.length * hookFit.size * 1.3 + 18 : 0;
    ...
    if (hookFit) {
      ...
      parts.push(textBlock(hookFit, {
        ...
        color: TOKENS.brand, anchor, lineGap: 1.3,
      }));
    }
  }
