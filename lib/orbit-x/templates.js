...
import {
  ...
  axisMotif, oppositionDiagram, ingressDiagram, PLANET_GLYPHS, ZODIAC_GLYPHS,
} from "./celestial.js";
...
export function normalizeDesign(design = {}, eventType = null, formatId = null) {
  return Object.freeze({
    aspect: ASPECTS[design.aspect] ? design.aspect : "square",
...
function footer(aspect, { page, total, dateShort, calculated }) {
  const A = ASPECTS[aspect];
  const y = A.footerY;
  const midParts = [];
  if (calculated) midParts.push("CALCULATED");
  if (dateShort) midParts.push(dateShort.toUpperCase());
  return `<text x="${A.safe.x + 10}" y="${y}" font-family="${TOKENS.sans}" font-size="28" letter-spacing="4" fill="${TOKENS.brand}">ORBIT AXIS</text>`
    + (midParts.length ? `<text x="${A.width / 2}" y="${y}" text-anchor="middle" font-family="${TOKENS.sans}" font-size="22" letter-spacing="3" fill="${TOKENS.muted}">${esc(midParts.join(" · "))}</text>` : "")
...
export function renderSlide(ctx) {
  ...
  const warnings = [];
  const role = ctx.slide?.role || (ctx.slideIndex === 0 ? "hero" : "explain");
  const centered = design.variant === 1;
  const S = A.safe;
  const parts = [`<rect width="${A.width}" height="${A.height}" fill="${TOKENS.bg}"/>`];
  ...
  if (role === "hero") {
    parts.push(eventBadge(badgeFor(ctx.eventType, ctx.facts), { x: anchorX, y: S.y + 10, font: 26 }));
    // Celestial anchor
    let visualBottom = S.y + 60;
    if (design.visuals.moon && moonState && ["full_moon", "new_moon", "daily_sky"].includes(ctx.eventType)) {
      const r = design.aspect === "portrait" ? 230 : 200;
      const cy = S.y + r + 90;
      ...
      parts.push(axisMotif({ x: S.x + 30, y: S.y + 460, length: 260 }));
      visualBottom = S.y + 300;
    }
    const fit = fitText(heading, { tiers: [96, 82, 68], maxLines: SAFE_LIMITS.headlineLines, width: S.w, font: "serif" });
    ...
    const hasDate = Boolean(dateShort);
    ...
    parts.push(textBlock(fit, { x: anchorX, y: headY, font: TOKENS.serif, color: TOKENS.ink, anchor, lineGap: 1.18 }));
    if (hasDate) {
      parts.push(`<text x="${anchorX}" y="${headY + (fit.lines.length - 1) * fit.size * 1.18 + 56}" text-anchor="${anchor}" font-family="${TOKENS.sans}" font-size="30" letter-spacing="2" fill="${TOKENS.body}">${esc(humanDate(ctx.facts?.date || ctx.facts?.local_date))}${ctx.facts?.approximate ? " (approximate)" : ""}</text>`);
    }
  }
    ...
    if ((design.density === "data" || role === "signal") && design.visuals.strip && Array.isArray(ctx.sky) && ctx.sky.length) {
    ...
    parts.push(axisMotif({ x: S.x + 30, y: A.footerY - 90, length: 180 }));
  }

  else if (role === "cta") {
    parts.push(`<text x="${A.width / 2}" y="${A.height / 2 - 120}" text-anchor="middle" font-family="${TOKENS.sans}" font-size="30" letter-spacing="6" fill="${TOKENS.brand}">ORBIT AXIS</text>`);
    ...
    parts.push(axisMotif({ x: S.x + 30, y: A.footerY - 90, length: 200 }));
  }
  ...
  parts.push(footer(design.aspect, { page: ctx.slideIndex + 1, total: ctx.total,
    dateShort, calculated: design.metadata.calculated }));
...
export function renderPost(post, ctx) {
  ...
  const slides = (post.slides || []).map((slide, i) => renderSlide({
    ...
    headline: post.headline, cta: post.cta,
