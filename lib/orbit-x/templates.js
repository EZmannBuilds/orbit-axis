...
/** How many lines of a given tier a region of `maxHeight` can hold. A block
 *  of n lines occupies (n - 1) gaps plus the cap height of the first line,
 *  which is how textBlock draws it. Without a height the line count is the
 *  only limit — the historical behaviour. */
function linesInHeight(size, lineGap, maxLines, maxHeight) {
  if (!Number.isFinite(maxHeight)) return maxLines;
  return Math.max(1, Math.min(maxLines, Math.floor((maxHeight - size) / (size * lineGap)) + 1));
}
 ...
 *
 * A region may also state the VERTICAL room it has (`maxHeight`, measured with
 * the same `lineGap` the caller will draw with). Height is the honest limit
 * wherever a body shares a slide with something drawn from facts — the sky
 * strip, the metadata row — because a line count alone cannot know how far
 * down the copy started. Without it, eight lines of 44px body fitted happily
 * and then printed straight through the positions grid.
 */
export function fitText(text, {
  tiers = [64, 56, 48], maxLines = 3, width = 860, font = "sans",
  lineGap = 1.3, maxHeight = Infinity,
} = {}) {
  ...
  const wrap = (size, limit) => {
    ...
    for (const word of words) {
      ...
      if (lines.length > limit) { fits = false; break; }
    }
    return { lines, fits };
  };
  for (const size of tiers) {
    const limit = linesInHeight(size, lineGap, maxLines, maxHeight);
    const { lines, fits } = wrap(size, limit);
    if (fits) return { lines: lines.map((l) => l.join(" ")), size, overflow: false };
  }
  // Smallest tier, hard-truncated at the region's line count — flagged, never
  // silent, and never allowed to run past the room it was given.
  const size = tiers[tiers.length - 1];
  const limit = linesInHeight(size, lineGap, maxLines, maxHeight);
  const { lines } = wrap(size, limit);
  return { lines: lines.slice(0, limit).map((l) => l.join(" ")), size, overflow: true };
}
...
export const SAFE_LIMITS = Object.freeze({
  ...
  // The hook is one line at feed scale. Past this it wraps, stops reading as
  // a hook, and starts reading as the paragraph slide one must not have.
  hookChars: 72, hookLines: 2,
});

/* ── What a slide can actually HOLD ──────────────────────────────────────────
   Every body region is (type tier × line count × the room left after the
   celestial marks). Those numbers used to live as literals inside the
   renderer while the desk advertised ONE flat character limit per format —
   so a brief promised 300 characters for a cover hook that can only ever
   draw 72, and the writer's work came back truncated. The specs below are
   the single source: renderSlide draws with them, bodyCapacity() reports
   them, and the two cannot drift apart. */
export const BODY_SPECS = Object.freeze({
  hero: Object.freeze({ tiers: [38, 34, 30], maxLines: SAFE_LIMITS.hookLines,
    font: "sans", lineGap: 1.3, inset: 20, cap: SAFE_LIMITS.hookChars }),
  fact: Object.freeze({ tiers: [44, 40, 36], maxLines: SAFE_LIMITS.bodyLines,
    font: "sans", lineGap: 1.4, inset: 0 }),
  symbolic: Object.freeze({ tiers: [42, 38, 34], maxLines: SAFE_LIMITS.bodyLines,
    font: "sans", lineGap: 1.45, inset: 60 }),
  reflection: Object.freeze({ tiers: [60, 52, 44], maxLines: 6,
    font: "serif", lineGap: 1.3, inset: 80 }),
  cta: Object.freeze({ tiers: [48, 42, 36], maxLines: 3,
    font: "serif", lineGap: 1.3, inset: 120 }),
  the_sky: Object.freeze({ tiers: [40, 36, 32], maxLines: 6,
    font: "sans", lineGap: 1.4, inset: 0 }),
});

/** Layout roles that share another role's body geometry. */
const BODY_SPEC_ALIAS = Object.freeze({
  explain: "fact", method: "fact", signal: "fact",
  takeaway: "reflection", your_sky: "the_sky",
});

export function bodySpecFor(role) {
  const layout = READING_LAYOUT_ROLE[role] || role;
  return BODY_SPECS[BODY_SPEC_ALIAS[layout] || layout] || BODY_SPECS.fact;
}

/* The positions grid is drawn from facts, so it owns its room: it sits a
   fixed distance above the footer and the copy fits ABOVE it. */
const STRIP_ROW_H = 66;
const STRIP_FOOT_GAP = 60;
const BODY_STRIP_GAP = 30;
const stripTopY = (A, rows) => A.footerY - STRIP_FOOT_GAP - rows * STRIP_ROW_H;

/** Where a fact-family body may run to: above the strip when one is drawn,
 *  otherwise above the metadata row. */
const factBodyBottom = (A, stripRows) => (stripRows
  ? stripTopY(A, stripRows) - BODY_STRIP_GAP
  : A.footerY - 104);

/** Greedy wrapping loses the tail of most lines at a word boundary. Nine
 *  tenths of the theoretical count is what prose reliably keeps — a budget a
 *  writer can trust is worth more than a ceiling they will trip over. */
const PACKING = 0.9;

/**
 * How many characters of prose a slide body will hold, from the same
 * geometry the renderer draws with. Reported to the desk and to the writer
 * brief so the limit on the page is the limit on the slide.
 *
 * @param {string} role semantic or layout role
 * @param {object} opts { aspect, stripRows, cursorY }
 */
export function bodyCapacity(role, { aspect = "portrait", stripRows = null, cursorY = null } = {}) {
  const A = ASPECTS[aspect] || ASPECTS.portrait;
  const spec = bodySpecFor(role);
  const layout = BODY_SPEC_ALIAS[READING_LAYOUT_ROLE[role] || role] || READING_LAYOUT_ROLE[role] || role;
  const size = spec.tiers[spec.tiers.length - 1];
  const width = A.safe.w - spec.inset;
  let maxHeight = Infinity;
  if (layout === "fact") {
    const rows = Number.isFinite(stripRows) ? stripRows : defaultStripRows(role, aspect);
    const top = Number.isFinite(cursorY) ? cursorY : A.safe.y + 90 + (role === "movements" || role === "signal" ? 230 : 0);
    maxHeight = factBodyBottom(A, rows) - top;
  }
  const lines = linesInHeight(size, spec.lineGap, spec.maxLines, maxHeight);
  const perLine = Math.max(4, Math.floor(width / (size * (CHAR_FACTOR[spec.font] || CHAR_FACTOR.sans))));
  const chars = Math.max(0, Math.floor(perLine * lines * PACKING));
  return spec.cap ? Math.min(spec.cap, chars) : chars;
}

/** The roles that carry the positions grid in a reading, and therefore write
 *  to a shorter region than their line count alone suggests. */
function defaultStripRows(role, aspect) {
  if (!["movements", "signal", "evidence", "key_dates", "method"].includes(role)) return 0;
  return Math.ceil((aspect === "portrait" ? 10 : 6) / 2);
}
...
export function renderSlide(ctx) {
  ...
  if (role === "hero") {
    ...
    // The copy is measured BEFORE the celestial anchor is drawn, because the
    // anchor is the element that can yield. A hero headline is bottom-clamped
    // above the footer, so a tall block reaches UP into the disc: on the daily
    // reading the word "light" landed on the lit half of the Moon — rendered
    // perfectly, unreadable. The disc still shows the illumination the engine
    // calculated; only its radius answers to the copy.
    ...
    const fit = fitText(heading, { tiers: heroTiers, maxLines: SAFE_LIMITS.headlineLines, width: S.w, font: heroFont, lineGap: 1.18 });
    ...
    // The hook line (Dev Update 5.4): the one line that earns the swipe. It
    // sits under the dateline in the accent, sized between the headline and
    // the metadata so the eye lands headline → hook → swipe.
    const hookText = String(ctx.slide?.body || "").trim();
    const hookSpec = BODY_SPECS.hero;
    const hookFit = hookText
      ? fitText(hookText, { tiers: hookSpec.tiers, maxLines: hookSpec.maxLines,
        width: S.w - hookSpec.inset, font: hookSpec.font, lineGap: hookSpec.lineGap })
      : null;
    if (hookFit && (hookFit.overflow || hookText.length > SAFE_LIMITS.hookChars)) {
      // Two lines of 30px type can technically hold more than a hook should
      // ever be. The budget is editorial before it is geometric, so the desk
      // is told at the same moment the advisory is — one number, one message.
      warnings.push(`The hook line is too long for slide 1 — keep it to about ${SAFE_LIMITS.hookChars} characters.`);
    }
    const hookH = hookFit ? hookFit.lines.length * hookFit.size * hookSpec.lineGap + 18 : 0;
    const blockH = (fit.lines.length - 1) * fit.size * 1.18 + (hasDate ? 62 : 0) + hookH;
    // A hook needs more clearance than a dateline does: it is the last thing
    // read before the swipe, and sitting it on top of the brand mark makes
    // both look like an accident.
    const bottomGap = hookFit ? 130 : 70;
    // The lowest a celestial mark may reach: one line's cap height above the
    // highest the copy block can ever start.
    const visualLimit = A.footerY - bottomGap - blockH - fit.size - 24;
    ...
    if (signalSign && ZODIAC_GLYPHS[signalSign]) {
      const y = S.y + 160;
      const size = Math.max(140, Math.min(design.aspect === "portrait" ? 300 : 250, visualLimit - y - 80));
      ...
      const top = S.y + (T.id === "orbit_signal" ? 140 : 90);
      // One radius, used to lay out AND to draw. They were two numbers before
      // — the layout reserved 285 and the disc drew its designed 200 — so the
      // clearance the hero thought it had was never the clearance on screen.
      const designed = design.aspect === "portrait" ? (T.id === "lunar_field" ? 260 : 230) : 200;
      const r = Math.min(designed, Math.floor((visualLimit - top - 60) / 2));
      if (r >= 72) {
        const cy = top + r;
        parts.push(moonDisc(moonState, { mode: "hero", radius: r, x: A.width / 2, y: cy,
          light: TOKENS.moonLight, dark: TOKENS.moonDark, rim: TOKENS.moonRim }));
        visualBottom = cy + r + 60;
      } else {
        // Below this the disc stops reading as the Moon. Absence is stated,
        // never a token circle drawn on top of the headline.
        warnings.push("Slide 1 has no room for the Moon — shorten the headline or the hook.");
      }
    ...
    const headY = Math.min(Math.max(visualBottom + fit.size, A.footerY - 200 - blockH), A.footerY - bottomGap - blockH);
    ...
    let afterHead = headY + (fit.lines.length - 1) * fit.size * 1.18;
    if (hasDate) {
      afterHead += 56;
      parts.push(`<text x="${anchorX}" y="${afterHead}" text-anchor="${anchor}" font-family="${TOKENS.sans}" font-size="30" letter-spacing="2" fill="${TOKENS.body}">${esc(periodLabel)}${ctx.facts?.approximate ? " (approximate)" : ""}</text>`);
    }
    if (hookFit) {
      // Brand purple in every family: the hook sits with the HEADLINE, below
      // any coloured band, and the headline is light everywhere. Keying the
      // colour off the template put near-black type on the dark half of
      // Orbit Signal, where it rendered perfectly and could not be seen.
      parts.push(textBlock(hookFit, {
        x: anchorX, y: afterHead + 52, font: TOKENS.sans,
        color: TOKENS.brand, anchor, lineGap: hookSpec.lineGap,
      }));
    }
  }
    ...
    // The strip is PLANNED before the copy is fitted, because it is drawn from
    // facts and the copy is not: the positions keep their room, and the body
    // fits the space that is left. Fitting the body first is what let seven
    // lines of prose print straight through the grid with no warning at all.
    const wantsStrip = (design.density === "data" || role === "signal" || semanticRole === "evidence")
      && design.visuals.strip && Array.isArray(ctx.sky) && ctx.sky.length;
    // Square shows six bodies — the template decides what is appropriate
    // (§10) — portrait has room for all ten.
    const shown = wantsStrip ? ctx.sky.slice(0, design.aspect === "portrait" ? 10 : 6) : [];
    const stripRows = shown.length ? Math.ceil(shown.length / 2) : 0;
    const spec = BODY_SPECS.fact;
    const fit = fitText(body, { tiers: spec.tiers, maxLines: spec.maxLines, width: S.w - spec.inset,
      font: spec.font, lineGap: spec.lineGap, maxHeight: factBodyBottom(A, stripRows) - cursorY });
    if (fit.overflow) warnings.push(`Slide ${ctx.slideIndex + 1} body exceeds the safe area — tighten the copy.`);
    parts.push(textBlock(fit, { x: S.x, y: cursorY + fit.size, font: TOKENS.sans, color: TOKENS.body, lineGap: spec.lineGap }));
    let stripShown = false;
    if (stripRows) {
      const strip = skyStrip(shown, { x: S.x, y: stripTopY(A, stripRows), columns: 2,
        cellWidth: S.w / 2, cellHeight: STRIP_ROW_H, showDegree: true, color: TOKENS.ink, subColor: TOKENS.muted, glyphSize: 34, font: 26 });
    ...
    const spec = BODY_SPECS.symbolic;
    const fit = fitText(body, { tiers: spec.tiers, maxLines: spec.maxLines, width: S.w - spec.inset,
      font: spec.font, lineGap: spec.lineGap, maxHeight: A.footerY - 104 - (S.y + 200) });
    if (fit.overflow) warnings.push(`Slide ${ctx.slideIndex + 1} body exceeds the safe area — tighten the copy.`);
    parts.push(textBlock(fit, { x: S.x, y: S.y + 200, font: TOKENS.sans, color: TOKENS.body, lineGap: spec.lineGap }));
  }
    ...
    const spec = BODY_SPECS.reflection;
    const fit = fitText(body, { tiers: spec.tiers, maxLines: spec.maxLines, width: S.w - spec.inset,
      font: spec.font, lineGap: spec.lineGap });
    if (fit.overflow) warnings.push(`Slide ${ctx.slideIndex + 1} reflection exceeds the safe area.`);
    const blockH = fit.lines.length * fit.size * spec.lineGap;
    const startY = (A.height - blockH) / 2 + fit.size * 0.6;
    parts.push(textBlock(fit, { x: A.width / 2, y: startY, font: TOKENS.serif, color: TOKENS.ink, anchor: "middle", lineGap: spec.lineGap }));
    ...
    const spec = BODY_SPECS.cta;
    const fit = fitText(body || ctx.cta || "", { tiers: spec.tiers, maxLines: spec.maxLines,
      width: S.w - spec.inset, font: spec.font, lineGap: spec.lineGap });
    if (fit.overflow) warnings.push(`Slide ${ctx.slideIndex + 1} closing line exceeds the safe area.`);
    parts.push(textBlock(fit, { x: A.width / 2, y: A.height / 2 - 40, font: TOKENS.serif, color: TOKENS.ink, anchor: "middle", lineGap: spec.lineGap }));
    ...
    const spec = BODY_SPECS.the_sky;
    const fit = fitText(body, { tiers: spec.tiers, maxLines: spec.maxLines, width: S.w - spec.inset,
      font: spec.font, lineGap: spec.lineGap, maxHeight: A.footerY - 104 - bodyStart });
    if (fit.overflow) warnings.push(`Slide ${ctx.slideIndex + 1} body exceeds the safe area.`);
    parts.push(textBlock(fit, { x: S.x, y: bodyStart, font: TOKENS.sans, color: TOKENS.body, lineGap: spec.lineGap }));
  }
