...
export function scoreCandidate(candidate, history = [], todayIso = "1970-01-01") {
  ...
  // The three readings all carry event_type "collective_reading", so a plain
  // type comparison made today's Daily post drain the novelty of this week's
  // Weekly and this month's Monthly — different products on different
  // cadences, penalising each other. `reading_type` (the column the readings
  // migration added, and which history() already selects) is what actually
  // identifies the family.
  const familyOf = (type, readingType) => (readingType ? `${type}:${readingType}` : type);
  const family = familyOf(candidate.eventType, candidate.readingType);

  const covered = history.filter((row) => row.event_key === candidate.eventKey
    && ["draft", "approved", "exported"].includes(row.status));
  const sameTypeRecent = history.filter((row) => familyOf(row.event_type, row.reading_type) === family
