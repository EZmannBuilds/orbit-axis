...
import {
  READING_TYPES, calculateReadingPeriod, buildReadingCandidate, editorialDate, editorialTimezone, readingFormatFor,
} from "./readings.js";
...
export async function handleOrbitXRoute(method, route, searchParams, body, auth, deps = {}) {
  ...
  try {
    if (route === "/api/orbit-x/candidates" && method === "GET") {
      ...
      const requestedDate = relative === "today" || relative === "tomorrow"
        ? editorialDate(new Date(), env.ORBIT_X_EDITORIAL_TIMEZONE, relative === "tomorrow" ? 1 : 0)
        : searchParams.get("date");
      const { context, skipped, setAside, ranked } = await scoredCandidates(store, requestedDate, tz, env);
      ...
      return { status: 200, body: { ok: true,
        localDate: context.local_date, editorialTimezone: context.timezone_name,
