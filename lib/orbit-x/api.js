...
import { buildScaffold, signFromTitle } from "./language.js";
...
function themesFor(candidate) {
  try {
    if (["full_moon", "new_moon", "daily_sky"].includes(candidate.eventType)) {
...
export async function handleOrbitXRoute(method, route, searchParams, body, auth, deps = {}) {
  ...
  try {
    ...
    if (route === "/api/orbit-x/manual" && method === "POST") {
      ...
      const { post: scaffold, suggestions } = buildScaffold(candidate, formatId, { themes: themesFor(candidate) });
    ...
    if (route === "/api/orbit-x/posts" && method === "POST") {
      ...
      const saved = await store.insert({
        ...
        edited_copy: post,
        template: post.design?.aspect === "portrait"
