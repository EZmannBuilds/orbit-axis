...
export function moonDisc(state, {
  mode = "inline", x = 0, y = 0, radius = null,
  ...
  // A mode carries a designed radius; a caller that must fit the disc into
  // measured room may state one instead. The disc is the same instrument at
  // any size — it is the illumination that may never be adjusted.
  const r = Number.isFinite(radius) && radius > 0
    ? radius : (MOON_MODES[mode] || MOON_MODES.inline).r;
