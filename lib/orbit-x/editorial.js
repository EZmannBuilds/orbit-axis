...
import { SAFE_LIMITS } from "./templates.js";
...
export function adviseRetention(post) {
  ...
  } else if (heroBody.length > SAFE_LIMITS.hookChars) {
    // 2. The opposite failure: a paragraph where a hook belongs (§41). The
    //    threshold is the RENDERER's hook limit, not a second opinion about
    //    it — they disagreed once, and copy that passed the advisory came out
    //    truncated mid-sentence on the slide.
    advisories.push({ path: "slides[0]", rule: "hero_paragraph",
      why: `slide 1's hook runs past ${SAFE_LIMITS.hookChars} characters and will be cut on the slide — a hook is one line; move the detail to slide 2` });
  }
