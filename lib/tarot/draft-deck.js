// Orbit Axis :: the DRAFT Tarot deck — drafted from tradition, and shipping.
//
...
//
// WHY IT IS STILL CALLED A DRAFT, AND WHAT REVIEW IT ACTUALLY PASSED
//
// This deck ships. It was approved by the owner on 2026-08-15 and again on
// 2026-08-18 for the expanded reversed meanings, and PROVENANCE below records
// `reviewed: true` with the reviewer, the date, and — the part that matters —
// `review_kind: "owner_approval"`. That is an owner's approval of the deck as
// a whole, not a line-by-line reading of all seventy-eight. Both are review;
// they are not the same review, and the field says which one this was so
// nobody has to guess later.
//
// This header used to say the opposite: `reviewed: false` on every card, and
// "cannot ship". That stopped being true the moment the deck shipped, and the
// comment was simply never updated — the kind of drift that is harmless until
// someone trusts it. `deckStatus()` is the authority here, not this paragraph:
// it refuses any deck that is short of seventy-eight cards, fails validation,
// or carries an unreviewed card, and production never passes allowUnreviewed.
//
// "DRAFT" in the filename and in DRAFT_CARDS is now about provenance rather
// than readiness. These meanings are public-domain-derived, not original, and
// the owner's stated plan is still to commission original ones from a writer.
// When those arrive they replace this file, the provenance names the author,
// and the review flag becomes theirs to set.
//
...
const PROVENANCE = Object.freeze({
  ...
  source: "Informed by A. E. Waite, The Pictorial Key to the Tarot (1911), and "
    ...
    + "Orbit Axis; not transcribed. Card artwork is the Waite-Smith plates "
    + "(1909), public domain, sourced from Wikimedia Commons.",
  tradition: "Rider-Waite-Smith",
  // Approved for release by the owner on 2026-08-15, as an owner's approval of
  // the deck rather than a line-by-line reading of all seventy-eight. That
  // distinction is recorded rather than smoothed over: it is what `reviewed`
  // means here, and a future reader of this file is entitled to know which
  // kind of review it was.
  //
  // Still true, and still the plan: these are public-domain-derived, not
  // original. When a writer is commissioned their meanings replace this file
  // and the provenance names them.
  reviewed: true,
  reviewed_by: "Erik Mann (owner)",
  // 2026-08-18: the owner approved the expanded reversed meanings ("ship it"),
  // the same gate the original meanings passed on 2026-08-15.
  reviewed_at: "2026-08-18",
  review_kind: "owner_approval",
});
...
const REVERSED = {
  "the-fool": ["Hesitation at the edge of a beginning, or a leap taken without looking at all. The Fool reversed is the gap between willingness and recklessness. It often shows up as research done twice and a first step still untaken — the plan polished in place of the leap.", "Are you holding back out of care, or out of fear?"],
  "the-magician": ["Capability not being used, or used to persuade rather than to make. The Magician reversed asks what the skill is actually pointed at. Watch for the day spent arranging the desk instead of doing the work it was arranged for.", "What are you talented at that you are wasting?"],
  "the-high-priestess": ["Ignoring what you already know. The High Priestess reversed is the quiet signal talked over by louder reasoning. It tends to appear as a decision that keeps needing one more opinion when the first instinct was already clear.", "What have you been explaining away?"],
  "the-empress": ["Tending everything except yourself, or growth stalled for want of care. The Empress reversed is depletion dressed as generosity. The tell is running on empty while insisting everyone else eats first.", "Who tends you?"],
  "the-emperor": ["Structure that has outlived its purpose, or rigidity mistaken for strength. The Emperor reversed is the rule nobody remembers making. It often looks like defending a routine you would never design today, simply because it is yours.", "Which of your rules would you not choose again?"],
  "the-hierophant": ["Convention followed past the point of belief, or rejected before it was understood. The Hierophant reversed is the inherited answer left unexamined. The tell is reciting a reason and noticing, mid-sentence, that you no longer believe it.", "What are you doing because it is done that way?"],
  "the-lovers": ["A choice avoided, or values pulling against each other. The Lovers reversed is misalignment between what you say matters and what you pick. It tends to surface as a calendar that contradicts the priorities you would list out loud.", "Where do your choices and your stated values disagree?"],
  "the-chariot": ["Effort in several directions at once, or momentum that has become its own reason. The Chariot reversed is drive without steering. Watch for busyness standing in for direction — many small wins accumulating toward nowhere in particular.", "What are you pushing hard at without checking where it goes?"],
  "strength": ["Force where patience would serve, or patience stretched into passivity. Strength reversed is the misjudged amount of pressure. It often shows up in the voice: pushing where listening would move things, or going quiet where a firm word was owed.", "Where are you gripping something too hard, or not hard enough?"],
  "the-hermit": ["Withdrawal that has become avoidance, or a refusal to step back at all. The Hermit reversed is solitude past its usefulness. The tell is calling it recharging long after the battery read full.", "Is your distance from this giving you clarity, or cover?"],
  "wheel-of-fortune": ["Resisting a turn that has already happened, or waiting for luck instead of acting. The Wheel reversed is a change not yet accepted. It tends to look like re-litigating a decision the world has already made for you.", "What has already changed that you are still arguing with?"],
  "justice": ["An account not squared, or a judgement made before the facts. Justice reversed is the story told to avoid the reckoning. Watch for the version of the story where you appear only as the reasonable one.", "What are you leaving out of your own account of this?"],
  "the-hanged-one": ["Stalling called reflection, or motion that refuses the pause. The Hanged One reversed is a suspension nobody is using. It often shows up as waiting for clarity that only the next move can produce.", "Is this a pause, or a delay you have renamed?"],
  "death": ["Holding on past the ending. Death reversed is the chapter maintained long after it finished. The tell is maintenance without affection — watering a plant you no longer like.", "What are you keeping alive out of habit?"],
  "temperance": ["Extremes taken in turn instead of a workable middle. Temperance reversed is the swing between too much and none at all. It tends to appear as all-or-nothing scheduling: the strict week, the abandoned month.", "Where are you living in extremes?"],
  "the-devil": ["The attachment seen clearly for the first time, or gripped tighter for being named. The Devil reversed is the moment a pattern becomes visible. Watch for the joke you make about the habit before anyone else can mention it.", "What have you started to see about a habit you defend?"],
  "the-tower": ["A collapse resisted, postponed, or survived. The Tower reversed is the structure shaking that has not fallen yet. It often shows up as effort spent keeping a thing looking stable rather than making it so.", "What are you propping up that wants to come down?"],
  "the-star": ["Hope that feels out of reach, or repair begun before rest was taken. The Star reversed is depletion where replenishment belongs. The tell is treating rest as a reward for finishing instead of the thing that lets you finish.", "What would restore you, if you let it?"],
  "the-moon": ["Confusion beginning to clear, or a fear examined and found smaller. The Moon reversed is uncertainty losing its grip. It tends to arrive as relief you have not trusted yet — the dread that shrank when you finally looked at it.", "What seems less frightening than it did?"],
  "the-sun": ["Warmth not reaching you, or brightness performed rather than felt. The Sun reversed is the good day that will not land. Watch for good news relayed in a flat voice, as if it happened to someone else.", "What is going well that you cannot feel?"],
  "judgement": ["A reckoning avoided, or judgement turned only on yourself. Judgement reversed is the honest look postponed. It often looks like an apology drafted and never sent, or a verdict on yourself no evidence could overturn.", "What are you refusing to reconsider?"],
  "the-world": ["Nearly finished, or finished without acknowledgement. The World reversed is the arc left one step short. The tell is starting the next thing early so the finished one never has to be felt.", "What is almost done, and what is the last step?"],

  "ace-of-wands": ["A spark that has not been acted on, or energy with nowhere to go. Delay rather than absence. It tends to show up as the tab left open for weeks — interest kept warm but never fed.", "What idea have you let go cold?"],
  "two-of-wands": ["Planning that has replaced starting. The map studied instead of used. Watch for the third revision of a plan whose first version was already good enough to try.", "What would starting badly teach you that more planning will not?"],
  "three-of-wands": ["Waiting badly, or checking on something that needs to be left alone. It often looks like refreshing for news that would arrive just as fast unwatched.", "What are you watching that would go better unwatched?"],
  "four-of-wands": ["A milestone passed without notice, or a celebration that felt hollow. The tell is reaching a marker you once longed for and treating it as a Tuesday.", "What have you achieved and not let yourself feel?"],
  "five-of-wands": ["Conflict avoided, or friction that has turned personal. It tends to surface as politeness in the meeting and friction everywhere after it.", "Which disagreement would go better said out loud?"],
  "six-of-wands": ["Recognition that does not land, or wanted too much. Watch for checking who noticed before deciding whether it counted.", "Whose approval are you working for?"],
  "seven-of-wands": ["Defending a position you no longer hold, or giving one up too early. It often shows up as arguing yesterday's position out of loyalty to having held it.", "Is this still worth defending?"],
  "eight-of-wands": ["Delay after momentum, or moving faster than understanding. The tell is answering fast to feel in motion, then spending the afternoon untangling the replies.", "What is moving too quickly to be understood?"],
  "nine-of-wands": ["Exhaustion carried as vigilance. Guarding after the threat has gone. It tends to look like reading tone into messages that carried none.", "What are you still braced against?"],
  "ten-of-wands": ["A load that could be set down or shared, and has not been. Watch for the help declined in less time than the task will take.", "What could you hand to someone else?"],
  "page-of-wands": ["Enthusiasm without follow-through, or interest talked about rather than tried. It often appears as the idea told to three people and shown to none.", "What have you been meaning to start?"],
  "knight-of-wands": ["Speed that has overshot, or an engine idling. The tell is arriving before your reasons do, or revving without choosing a road.", "Where has hurry cost you more than waiting would have?"],
  "queen-of-wands": ["Confidence performed rather than felt, or warmth withheld. It tends to show up as taking the temperature of a room before deciding what you feel about it.", "Where are you acting more certain than you are?"],
  "king-of-wands": ["Direction imposed rather than shared, or vision without follow-through. Watch for a vision delegated so completely that no one can find you inside it.", "Are you leading, or only deciding?"],

  "ace-of-cups": ["Feeling held back, or offered and not received. It often looks like changing the subject at the exact moment a conversation turns warm.", "What feeling are you keeping at arm's length?"],
  "two-of-cups": ["Connection out of balance, with one side reaching further. The tell is keeping score quietly and calling it noticing.", "Where are you giving more than is coming back?"],
  "three-of-cups": ["Company without closeness, or withdrawal from people who would welcome you. It tends to appear as leaving the group chat unread while feeling left out of it.", "Who have you not called?"],
  "four-of-cups": ["Noticing the offer at last, or refusing it again. Watch for dismissing the offer first and wondering about it after.", "What have you turned down twice?"],
  "five-of-cups": ["Grief loosening, or loss still filling the whole view. It often shows up as an inventory that lists the losses in detail and the remainder not at all.", "What remains that you have not counted?"],
  "six-of-cups": ["The past used as a place to live rather than to visit. The tell is comparing every present thing to a remembered version that has quietly improved with age.", "What is nostalgia costing you now?"],
  "seven-of-cups": ["Choice made at last, or options multiplied to avoid one. It tends to look like adding a fourth option to avoid ranking the first three.", "Which option would you pick if the others vanished?"],
  "eight-of-cups": ["Staying past the point of meaning, or leaving without knowing why. Watch for the reasons to stay getting shorter while the list of them gets longer.", "What are you staying for?"],
  "nine-of-cups": ["Satisfaction that does not satisfy, or pleasure taken instead of what was wanted. It often arrives as the treat that was supposed to stand in for the thing you actually wanted.", "What are you settling for?"],
  "ten-of-cups": ["Belonging idealised, or a togetherness that costs someone something. The tell is tidying the picture of the day rather than being in the day.", "What does your picture of a good life leave out?"],
  "page-of-cups": ["Feeling guarded before it is understood, or sensitivity taken as fragility. It tends to show up as deciding what a feeling means before letting it finish arriving.", "What are you protecting yourself from feeling?"],
  "knight-of-cups": ["Following a feeling past where it was useful, or refusing to follow one at all. Watch for the gesture planned in more detail than the conversation it was meant to open.", "Where is a feeling steering more than it should?"],
  "queen-of-cups": ["Holding others' feelings until there is no room for your own. It often looks like knowing everyone's weather and being unable to report your own.", "Whose feelings are you carrying that are not yours?"],
  "king-of-cups": ["Composure that has become suppression. The tell is 'I'm fine' delivered with the calm of a closed door.", "What are you managing instead of feeling?"],

  "ace-of-swords": ["Clarity not yet reached, or a truth used as a weapon. It tends to appear as the sentence you keep almost saying because its second half is still forming.", "What are you close to understanding?"],
  "two-of-swords": ["The stalemate breaking, or a decision avoided so long it decided itself. Watch for the option that expired while you were weighing it — that was a decision too.", "What has your indecision already chosen?"],
  "three-of-swords": ["Hurt beginning to ease, or a wound kept open by retelling. It often shows up as the retelling that has grown smoother than the memory it came from.", "What story about this do you repeat?"],
  "four-of-swords": ["Rest refused, or rest that has become retreat. The tell is resting with the phone in hand, which is standing by, not resting.", "When did you last stop?"],
  "five-of-swords": ["A win reconsidered, or a fight nobody has ended. It tends to look like winning the exchange and replaying it anyway.", "What would it take to stop being right about this?"],
  "six-of-swords": ["A transition stalled, or made without the reckoning it needed. Watch for the new chapter narrated in the old chapter's voice.", "What are you carrying that you meant to leave?"],
  "seven-of-swords": ["Something concealed coming to light, or self-deception rather than deception. It often arrives as the small edit to the story that makes your part easier to tell.", "What are you not admitting to yourself?"],
  "eight-of-swords": ["Restriction loosening, or bindings maintained from the inside. The tell is a rule you obey that no one, checked closely, is enforcing.", "What could you test that you have assumed?"],
  "nine-of-swords": ["Worry easing, or anxiety that has outlived its cause entirely. It tends to show up at three in the morning, arguing a case the daylight already dismissed.", "Is the fear about now, or about then?"],
  "ten-of-swords": ["The worst passed, or an ending refused after it has arrived. Watch for revisiting the ending to see if it ends differently — it has not, and that is information.", "What is over that you keep returning to?"],
  "page-of-swords": ["Curiosity turned to suspicion, or questions asked to test rather than to learn. It often looks like a question sharpened until the answer no longer matters.", "Are you asking to find out, or to be proved right?"],
  "knight-of-swords": ["Certainty ahead of evidence, or conviction lost mid-argument. The tell is a conclusion sprinting ahead while its evidence walks.", "What would change your mind?"],
  "queen-of-swords": ["Directness that has hardened, or honesty softened into vagueness. It tends to appear as candour saved for others and vagueness kept for yourself.", "Where is your kindness costing someone the truth?"],
  "king-of-swords": ["Judgement given where it was not asked for, or withheld where it was. Watch for a ruling issued on a matter still waiting for its facts.", "Whose decision is this actually?"],

  "ace-of-pentacles": ["An opportunity not taken up, or one taken for the wrong reason. It often shows up as the practical first step priced at five minutes and postponed for a month.", "What practical opening have you let pass?"],
  "two-of-pentacles": ["Juggling that has become dropping. More commitments than hands. The tell is a calendar that balances only if nothing anywhere runs long.", "What has to go?"],
  "three-of-pentacles": ["Contribution unrecognised, or work done alone that needed others. It tends to look like doing it alone because explaining it felt slower — it was not.", "Whose help have you not asked for?"],
  "four-of-pentacles": ["Grip loosening, or security pursued past the point of use. Watch for keeping the reserve so safe it does none of the things reserves are for.", "What would you lose by holding less tightly?"],
  "five-of-pentacles": ["Help accepted at last, or hardship carried in silence. It often appears as walking past open doors on the principle that this must be endured.", "Who would help if you told them?"],
  "six-of-pentacles": ["Giving with a string attached, or receiving that feels like debt. The tell is generosity with a ledger where a gift was meant to be.", "What is expected in return here?"],
  "seven-of-pentacles": ["Effort continued without assessment, or abandoned just before it paid. It tends to show up as tending the plot on schedule without asking whether the crop is one you still want.", "Is this worth another season?"],
  "eight-of-pentacles": ["Practice without attention, or skill chased for its own sake. Watch for the hundredth repetition performed exactly like the tenth.", "What are you repeating without improving?"],
  "nine-of-pentacles": ["Self-sufficiency that has become isolation. It often looks like a life arranged so well that asking anyone for anything would disarrange it.", "What have you built that you cannot share?"],
  "ten-of-pentacles": ["Security assumed rather than built, or built for people who did not ask for it. The tell is building the legacy in place of the visit.", "Who is this for?"],
  "page-of-pentacles": ["Study without practice, or practice without patience. It tends to appear as the fourth course purchased before the first was practised.", "What are you learning that you have not yet used?"],
  "knight-of-pentacles": ["Thoroughness that has become stalling. Watch for the checklist lengthening at exactly the rate the finish approaches.", "What is finished enough to move on from?"],
  "queen-of-pentacles": ["Caring for everyone practically and no one warmly, including yourself. It often shows up as a full pantry, a made bed, and no idea what you yourself came in for.", "What do you need that you have not arranged for?"],
  "king-of-pentacles": ["Resources held rather than used, or competence that has stopped being curious. The tell is stewardship without appetite — everything maintained, nothing ventured.", "What would you do with this if you were not protecting it?"],
};
...
/**
 * The version this deck draws under.
 *
 ...
 * version says it did.
 */
export const DRAFT_DECK_VERSION = "1.1.0";
