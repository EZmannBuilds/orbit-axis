 ...
 */
export function drawDailyCard({ deck, localDate, timezone = "UTC", deckVersion = DECK_VERSION, subject = "anonymous", reversals = false }) {
  const seed = dailySeed({ localDate, timezone, deckVersion, subject });
  const [card] = drawCards(deck, 1, seed);
  return {
    card,
    orientation: reversals && isReversed(seed, 0) ? "reversed" : "upright",
    draw: {
      spread_type: "daily",
...
export function drawSpread({ deck, spreadType, deckVersion = DECK_VERSION, seed = manualSeed(), reversals = false }) {
  ...
  return {
    cards: cards.map((card, i) => ({
      ...
      card,
      orientation: reversals && isReversed(seed, i) ? "reversed" : "upright",
    })),
    draw: {
