 ...
 *
 * Injectable so tests can supply the fixture deck, but production has exactly
 * one answer and it is the empty PRODUCTION_CARDS. There is no environment
 * variable that swaps a deck in: content arrives by code change and review,
 * which is the ceremony the licensing note requires.
 */
export function resolveDeck({ deck = PRODUCTION_CARDS, deckVersion = DECK_VERSION } = {}) {
  return { deck, deckVersion };
}
 ...
 * worked.
 */
export function assertDeckReady({ deck = PRODUCTION_CARDS } = {}) {
...
export function validateReadingPayload(payload, { deck, deckVersion = DECK_VERSION } = {}) {
  ...
  const resolved = cards.map((entry, index) => {
    ...
    seen.add(slug);
    return { position: positions[index], card: presentCard(card) };
  });
 ...
 */
export function dailyCard({ deck, deckVersion = DECK_VERSION, timezone = "UTC", subject = "anonymous", now = new Date() } = {}) {
  assertDeckReady({ deck });
  const zone = validateTimezone(timezone);
  const localDate = localDateForZone(now, zone);
  const { card, draw } = drawDailyCard({ deck, localDate, timezone: zone, deckVersion, subject });
  return { spread_type: "daily", question: null, cards: [{ position: SPREAD_POSITIONS.daily[0], card: presentCard(card) }], draw };
}

/** A manual one- or three-card reflection. Drawn now, not derived. */
export function manualReading({ deck, deckVersion = DECK_VERSION, spreadType, question = null, seed, timezone = "UTC", now = new Date() } = {}) {
  assertDeckReady({ deck });
  ...
  const zone = validateTimezone(timezone);
  const { cards, draw } = drawSpread({ deck, spreadType: type, deckVersion, ...(seed ? { seed } : {}) });
  return {
    spread_type: type,
    question: cleanQuestion,
    cards: cards.map(({ position, card }) => ({ position, card: presentCard(card) })),
