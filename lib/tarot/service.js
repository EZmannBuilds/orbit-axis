...
import { DRAW_CONTRACT_VERSION, drawDailyCard, drawSpread, isLocalDate, shownOrientation } from "./draw.js";
 ...
 *
 * PRODUCTION HAS EXACTLY ONE ANSWER and no environment variable swaps a deck
 * in. Content arrives by code change and review, which is the ceremony the
 * licensing note requires. That part has not changed.
 *
 * WHAT HAS CHANGED: this used to say production served "the empty
 * PRODUCTION_CARDS" and that the draft "declares itself unreviewed". Neither
 * is true since the deck shipped. PRODUCTION_CARDS *is* DRAFT_CARDS — the same
 * seventy-eight objects, at the same version — so today both branches below
 * return the same deck and the split is vestigial.
 *
 * It is kept rather than collapsed because it is the seam a commissioned deck
 * arrives on: when a writer's meanings land in PRODUCTION_CARDS, production
 * takes them and preview can keep reading the draft to compare, with the two
 * version strings keeping the draws openly distinct rather than silently the
 * same. Collapsing it now would mean rebuilding it then.
 *
 * `assertDeckReady` still gates every path, and production never passes
 * allowUnreviewed — so an unreviewed deck cannot ship by being forgotten, only
...
export function resolveDeck({ deck, deckVersion, env = process.env } = {}) {
  ...
  // Production reads the production deck and everywhere else reads the draft.
  // They are currently the same object at the same version, so this branch
  // decides nothing today; it decides everything the day a commissioned deck
  // lands in PRODUCTION_CARDS. The draft carries its own version so that a card
  // drawn from it is then openly a different draw rather than silently the same.
...
export function validateReadingPayload(payload, { deck, deckVersion = DECK_VERSION } = {}) {
  ...
  return {
    ...
    draw: {
      ...
      // FALSE for every spread, daily included. This once read
      // `spreadType === "daily"`, from the era when the daily card was a pure
      // function of (date, timezone, deck version, account) and could honestly
      // be recomputed. It is a real draw now — drawDailyCard says
      // `reproducible: false` and the seed is not kept — so a saved daily
      // reading claiming otherwise was history contradicting the draw that
      // made it. Nothing in the app recomputes from this flag; the account
      // export publishes it (lib/account/export.js), which is precisely why it
      // has to be true.
      reproducible: false,
    },
  };
...
export async function dailyCard({
  ...
  if (store) {
    ...
    if (stored && stored.card_slug !== card.slug) {
      ...
      if (winner) {
        return dailyReading({
          card: winner, orientation: stored.orientation,
          localDate, zone, deckVersion, reversals, source: "remembered",
        });
      }
    }
 ...
 *
 * `orientation` is always the DRAWN orientation. Two different things are done
 * with it here, and keeping them apart is the whole point: the reader is shown
 * `shownOrientation(...)`, which honours their setting, while `remember` and
 * the stored row keep the orientation the card actually landed in. A
 * signed-out reader echoes `remember` back on their next request, so writing
 * the shown value there would lose the reversal in localStorage exactly as the
 * old draw-time gate lost it in the database.
 */
function dailyReading({ card, orientation, localDate, zone, deckVersion, reversals, source, draw = null }) {
  const shown = shownOrientation(orientation, reversals);
  return {
    spread_type: "daily",
    question: null,
    cards: [{ position: SPREAD_POSITIONS.daily[0], card: presentCard(card, { orientation: shown }) }],
    draw: draw ?? {
      spread_type: "daily",
      contract_version: DRAW_CONTRACT_VERSION,
      deck_version: deckVersion,
...
export function manualReading({ deck, deckVersion = DECK_VERSION, spreadType, question = null, seed, timezone = "UTC", now = new Date(), reversals = false } = {}) {
  ...
  return {
    ...
    cards: cards.map(({ position, card, orientation }) => ({
      position,
      card: presentCard(card, { orientation: shownOrientation(orientation, reversals) }),
    })),
