 ...
 */
export function dailyCard({ deck, deckVersion = DECK_VERSION, timezone = "UTC", subject = "anonymous", now = new Date(), reversals = false } = {}) {
  assertDeckReady({ deck });
  const zone = validateTimezone(timezone);
  const localDate = localDateForZone(now, zone);
  const { card, orientation, draw } = drawDailyCard({ deck, localDate, timezone: zone, deckVersion, subject, reversals });
  return {
    spread_type: "daily", question: null,
