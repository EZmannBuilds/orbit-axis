...
import {
  ...
  cardBySlug, deckStatus, deckSupportsReversals, presentCard,
} from "./deck.js";
...
export async function dailyCard({
  ...
  // The reader's setting only holds if every card can honour it. A deck where
  // any card lacks its reversed half must serve upright, because presenting a
  // reversal with no authored meaning is a blank where an answer should be.
  reversals = reversals && deckSupportsReversals(deck);
...
export function manualReading({ deck, deckVersion = DECK_VERSION, spreadType, question = null, seed, timezone = "UTC", now = new Date(), reversals = false } = {}) {
  ...
  const zone = validateTimezone(timezone);
  // Same rule as the daily card: reversals only when every card carries both halves.
  const effectiveReversals = reversals && deckSupportsReversals(deck);
  const { cards, draw } = drawSpread({ deck, spreadType: type, deckVersion, reversals: effectiveReversals, ...(seed ? { seed } : {}) });
  return {
    spread_type: type,
