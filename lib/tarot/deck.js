...
//
// THE PRODUCTION DECK IS NO LONGER EMPTY. It was, for as long as this comment
// claimed it — the contract came first and the content followed. PRODUCTION_CARDS
// now holds the seventy-eight cards of draft-deck.js, written for Orbit Axis
// from the public-domain tradition, each with artwork from the Waite-Smith
// plates and provenance recording an owner's approval. Everything downstream
// — the draw, the API, the panel — is still built and tested against the
// contract using a fixture deck that says, in its own data, that it is a
// fixture, which is what keeps the tests honest about content they do not own.
//
...
// is switched on by accident. It is what let the deck ship the day it was
// complete and reviewed, with no other change.

/**
 * The deck version.
 *
 * It is recorded on every saved reading, so a reflection kept a year from now
 * still says which deck it was drawn from. Re-authoring the deck earns a new
 * version, and a saved reading keeps the text it was actually drawn with.
 */
import { DRAFT_CARDS as DECK_CARDS } from "./draft-deck.js";

// 1.1.0: every reversed meaning gained a grounding third sentence — how the
// reversal tends to show up in an ordinary day. Slugs unchanged; a saved
// reading keeps the text it was drawn with. The daily draw re-keys once,
// by design: a different deck version is a different deck.
export const DECK_VERSION = "1.1.0";
 ...
 *                    face is drawn from when a deck has no artwork. The shipped
 *                    deck does have artwork now, but the face is not dead code:
 *                    it is what a reader sees before the image arrives, and
 *                    `image` is still optional on the contract.
 * `upright_meaning`  the authored interpretation. `reversed_meaning` is NOT
 *                    required, so that a deck can be shipped without reversals
 *                    — the shipped deck happens to carry both halves, and
 *                    `deckSupportsReversals()` is how the app asks rather than
 *                    assuming.
 * `reflection_prompt`
 ...
 * THE PRODUCTION DECK.
 *
 * Seventy-eight cards, approved for release by the owner on 2026-08-15. They
 * are public-domain-derived rather than original — prose written for Orbit,
 * informed by Waite (1911) and the Waite-Smith deck (1909), with the artwork
 * being those plates directly. The provenance on every card says so, and the
 * intention to commission original meanings is unchanged; this is the interim
 * position, recorded as such rather than quietly becoming permanent.
 *
 * This was an empty array for the whole build. Everything downstream was
 * written and tested against the contract while it was empty, which is why
 * filling it required no other change.
 */
export const PRODUCTION_CARDS = DECK_CARDS;
...
export function validateCard(card, { index = 0 } = {}) {
  ...
  // A card that offers reversals must offer BOTH halves. A reversed meaning
  // with no prompt would leave the reader a paragraph and no question, which
  // is the one shape this feature is not.
  const hasReversed = Boolean(card.reversed_meaning) || Boolean(card.reversed_prompt);
  if (hasReversed && !(card.reversed_meaning && card.reversed_prompt)) {
    findings.push(`${at}: reversed cards need both reversed_meaning and reversed_prompt`);
  }

  for (const field of ["upright_meaning", "reflection_prompt", "reversed_meaning", "reversed_prompt"]) {
 ...
 */
/** Does every card in this deck carry reversed content? */
export function deckSupportsReversals(cards = PRODUCTION_CARDS) {
  return cards.length > 0 && cards.every((card) => card?.reversed_meaning && card?.reversed_prompt);
}
 ...
 */
export function presentCard(card, { orientation = "upright" } = {}) {
  if (!card) return null;
  const reversed = orientation === "reversed" && Boolean(card.reversed_meaning);
  return {
    // The meaning FOLLOWS the orientation rather than the client choosing
    // which of two fields to read. One card, one meaning, decided where the
    // draw happens — a client that picked would eventually disagree with what
    // was saved.
    orientation: reversed ? "reversed" : "upright",
    upright_meaning: reversed ? card.reversed_meaning : card.upright_meaning,
    reflection_prompt: reversed ? card.reversed_prompt : card.reflection_prompt,
    slug: card.slug,
    name: card.name,
    arcana: card.arcana,
    suit: card.suit ?? null,
...
export function imageUrl(path, env = process.env) {
  if (!path) return null;
  // An explicit base wins, so the artwork can move to a CDN later without a
  // client release.
  const base = env.ORBIT_TAROT_IMAGE_BASE_URL;
  if (base) return `${String(base).replace(/\/+$/, "")}/${String(path).replace(/^\/+/, "")}`;

  // Otherwise the app's own origin. The cards ship WITH the app: public/ is
  // what Capacitor bundles into the iOS binary, so on a phone these are local
  // files — instant, available with no signal, and costing no egress. On the
  // web they are ordinary static assets on the CDN.
  //
  // An earlier version required the base and served from object storage. That
  // was the wrong call twice over: it made the phone fetch over the network
  // something it could have carried, and the size argument behind it compared
  // 6.4MB against the wrong number.
  return `/images/tarot/cards/${String(path).split("/").pop()}`;
}
